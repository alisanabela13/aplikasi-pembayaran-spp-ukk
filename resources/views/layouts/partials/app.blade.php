...
<head>
  ...
  <title>SPP-Ku | @yield('title')</title>
...
<script>
  ...
  $(function(){ 
    function deleteWithForm($form) {
      Swal.fire({
        title: 'Apakah anda yakin ingin Memverifikasi Data?',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
    ...
    $(document).on('click', function(event) {
      if ( (event.target.tagName === 'BUTTON' && $(event.target).parent().hasClass('form-verif')) ) {
        event.preventDefault();
        deleteWithForm($(event.target).parent());
        return;
      }

      if ( (event.target.tagName === 'I' && $(event.target).parent().hasClass('btn-verif')) ) {
        event.preventDefault();
        deleteWithForm($(event.target).parent().parent());
        return;
      }
